...
def test_plan_research_empty_topic_raises():
    planner = ResearchPlanner()
    with pytest.raises(ValueError):
        asyncio.run(planner.plan_research(""))


def test_analyze_topic_complexity_scoring():
    planner = ResearchPlanner()
    score = asyncio.run(planner.analyze_topic_complexity("two words"))
    assert score == 2


def test_plan_research_caching():
    cache = CacheService()
    planner = ResearchPlanner(cache)
    asyncio.run(planner.plan_research("topic"))
    assert "plan:topic" in planner.cache._local_cache
