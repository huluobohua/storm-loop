...
class PRISMAScreener:
    ...
    Include and exclude keyword patterns are scored and compared against a
    threshold to decide on inclusion or exclusion. When scores are tied or below
    the threshold, the result is ``unsure``.
    """
    ...
    def _scores(self, inc_hits: int, exc_hits: int) -> Tuple[float, float]:
        inc_score = (
            ...
            if self.include_patterns else 0.0
        )
        exc_score = (
            exc_hits / len(self.exclude_patterns)
            if self.exclude_patterns else 0.0
        )
    ...
    def _conflict_result(
        ...
        """Resolve conflicts when scores do not clearly include or exclude.

        Excludes only when exclude hits outnumber include hits, includes when
        include hits outnumber exclude hits, otherwise returns an unsure result
        for tie scenarios or low scores.
        """
