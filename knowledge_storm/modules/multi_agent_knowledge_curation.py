...
class MultiAgentKnowledgeCurationModule(KnowledgeCurationModule):
    ...
    async def _safely_execute_task(self, agent_id, task, default, task_name):
        """Execute a task using the coordinator with error handling."""
        try:
            return await self.coordinator.distribute_task(agent_id, task)
        except Exception as e:  # pragma: no cover - log and continue
            logger.warning(f"%s failed for %s: %s", task_name, task, e)
            return default
