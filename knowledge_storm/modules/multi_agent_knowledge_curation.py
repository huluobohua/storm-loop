...
class MultiAgentKnowledgeCurationModule(KnowledgeCurationModule):
    ...
    async def _safely_execute_task(self, agent_id: str, task: str, task_name: str):
        """Execute task for agent and return fallback message on failure."""
        try:
    ...
    async def research(self, topic, ground_truth_url="", callback_handler=None, max_perspective=0, disable_perspective=True, return_conversation_log=False):
        ...
        plan_result = await self._safely_execute_task(self.planner.agent_id, topic, "Planning")
        research_result = await self._safely_execute_task(self.researcher.agent_id, topic, "Research")
        critique_result = await self._safely_execute_task(self.critic.agent_id, research_result, "Critique")
        verify_result = await self._safely_execute_task(self.verifier.agent_id, research_result, "Verification")
